...
full_data = subset(all_infant_data, Use == "Y" & last_swab_day > 150 &  PatientID2 != "O") %>%
  ...
  mutate(
    ...
    orig_infection_day = days2[days_orig == 0]
    )
```
...
adj_tbl = subset(full_data, adjusted) %>% group_by(PatientID2, orig_infection_day) %>%
               summarize() %>% rename(days_relative_est_infection_time = orig_infection_day)
  
print(xtable(adj_tbl, caption = "adjusted times"), include.rownames = F, comment = F)
...
if(save_output_figs){
  ggsave("../figures/supplementary/excluded_infants_fullO.pdf", supp_fig1, height = 4, width = 6)
